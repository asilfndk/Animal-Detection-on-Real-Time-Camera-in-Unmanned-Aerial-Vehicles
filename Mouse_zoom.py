...
def calculate_pixel_coordinates(lat, lon, corner_coords, image_dimensions, zoom_factor=1.0, zoom_center=None):
    ...
    if zoom_center:
        ...
        x = (x - zoom_x) * zoom_factor + zoom_x
        y = (y - zoom_y) * zoom_factor + zoom_y
    else:
        x = x * zoom_factor
        y = y * zoom_factor
...
def mouse_callback(event, x, y, flags, param):
    global zoom_center, zoom_factor
    if event == cv2.EVENT_LBUTTONDOWN:
        zoom_center = (x, y)
        zoom_factor = 2.0  # Zoom level can be adjusted as required
    elif event == cv2.EVENT_RBUTTONDOWN:
        zoom_center = None
        zoom_factor = 1.0 # Zoom level can be adjusted as required
...
while cap.isOpened():
    ...
    if zoom_center:
        ...
        start_x = max(x - 50, 0)
        start_y = max(y - 50, 0)
        end_x = min(x + 50, width)
        end_y = min(y + 50, height)
        
        # 100x100 piksellik alanı kırp
    ...
    for animal_key, animal_data in data['animal_coords'].items():
        ...
        animal_pixel_x, animal_pixel_y = calculate_pixel_coordinates(animal_x, animal_y, rect_coords, (width, height), zoom_factor, zoom_center)
        if 0 <= animal_pixel_x < width and 0 <= animal_pixel_y < height:
            ...
            target_x, target_y = calculate_pixel_coordinates(animal_x, animal_y, rect_coords, (width, height), zoom_factor, zoom_center)
